...
async function search (regex, data) {
    let a;
...
function searchSync (regex, data) {
    let a;
    while ((a = regex.exec(data)) !== null) {
        if (a.index === regex.lastIndex) {
            regex.lastIndex++;
...
function replaceAll(str, replaceInfo) {
    replaceInfo.forEach((element) => {
        str = str.replace(new RegExp(element[0].replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), element[1]);
    });
    return str;
}
